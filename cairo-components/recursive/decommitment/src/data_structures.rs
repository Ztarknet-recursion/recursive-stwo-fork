...
mod tests {
    ...
    use cairo_plonk_dsl_hints::{CairoFiatShamirHints, DecommitmentHints};
    use circle_plonk_dsl_constraint_system::{var::AllocationMode, ConstraintSystemRef};
    use std::path::PathBuf;

    #[test]
    fn test_preprocessed_trace_query_result_consistency() {
        ...
        let preprocessed_trace =
            DecommitmentHints::read_preprocessed_trace(&fiat_shamir_hints, &proof);
        assert!(
            !preprocessed_trace.is_empty(),
            "Preprocessed trace should not be empty"
        );

        let cs = ConstraintSystemRef::new();
        let preprocessed_result = &preprocessed_trace[0];
        let preprocessed_result_var = PreprocessedTraceQueryResultVar::new_variables(
            &cs,
            preprocessed_result,
            AllocationMode::Witness,
        );

        let column_hashes = preprocessed_result.compute_column_hashes();
        let column_hashes_var = preprocessed_result_var.compute_column_hashes();

        assert_eq!(
            column_hashes.len(),
            column_hashes_var.len(),
            "Column hashes maps should have the same length"
        );

        for (idx, hash) in column_hashes.iter() {
            let hash_var = column_hashes_var.get(idx).expect(&format!(
                "Column hash for index {} should exist in Var version",
                idx
            ));
            assert_eq!(
                hash.0, hash_var.value,
                "Column hash values should match for index {}",
                idx
            );
        }
    }
